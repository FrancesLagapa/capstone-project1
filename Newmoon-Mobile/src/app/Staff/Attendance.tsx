  ...
  const submitAutoAttendance = async () => {
    ...
    try {
      ...
      if (shouldTimeIn) {
        ...
        // ✅ Extract the actual attendance record
      ...
      if (shouldTimeOut && attendance?.id) {
        ...
        const response = await submitAttendanceTimeOut(attendance.id, body);
        if (response.queued) {
          setAttendance({
            ...attendance,
            time_out: time,
        ...
        <View className="mx-4 mt-1 rounded-3xl overflow-hidden border border-emerald-400/30 bg-zinc-900" style={{ minHeight: 390 }}>
          ...
          <View style={{ minHeight: 340 }}>
            <CameraView
              ...
              enableTorch={torchOn}
            />
            <View className="absolute left-8 right-8 top-10 bottom-10 border-2 border-emerald-400/70 rounded-3xl" pointerEvents="none"></View>
