  ...
  const submitMutation = useMutation({
    mutationFn: async (data: { product_id: number; quantity: number; reason: string | null }) => {
      const result = await submitOrQueue({
        ...
        data,
        type: 'supply-request',
        label: `Stock request x${data.quantity}`,
      });
      return result;
    },
    onSuccess: async (result) => {
      const message = result.queued
        ...
        : 'Your supply request has been submitted';
  ...
  if (loading && !refreshing) {
    return (
      <SafeAreaView className="flex-1 justify-center items-center bg-black">
        ...
        <Text className="mt-4 text-gray-400 font-medium">Loading Stock Requests...</Text>
      </SafeAreaView>
    );
  }
      ...
      >
        ...
        <View className="pt-12 pb-8 px-6 rounded-b-3xl" style={{ backgroundColor: '#10B981' }}>
          <View className="flex-row justify-between items-center mb-6">
            <View>
              ...
              <Text className="text-white text-3xl font-bold">Stock Requests</Text>
              <Text className="text-green-100 text-sm mt-1">Request and track your stock requests</Text>
            </View>
            <TouchableOpacity 
          ...
          {statistics?.data && (
            <View className="flex-row gap-3">
              <View className="flex-1 bg-white/10 rounded-xl p-4 border border-white/20">
                ...
                <Text className="text-white text-xl font-bold">{statistics.data.total_quantity_approved}</Text>
              </View>
              <View className="flex-1 bg-white/10 rounded-xl p-4 border border-white/20">
        ...
        <View className="px-6 mt-6">
          ...
          >
            ...
            <Text className="text-white font-bold text-lg ml-2">Request Stock</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="px-6 mt-6">
          <View className="flex-row justify-between items-center mb-4">
            ...
            <Text className="text-gray-400 text-sm">{requests.length} request(s)</Text>
          </View>
          ...
          {requests.length === 0 ? (
            <View className="bg-gray-900 rounded-xl p-8 items-center border border-gray-800">
              ...
              <Text className="text-gray-300 font-medium text-lg">No Requests Yet</Text>
            ...
            requests.map((request: StockRequest) => (
              <View key={request.id} className="bg-gray-900 rounded-xl p-4 mb-3 border border-gray-800">
                <View className="flex-row justify-between items-start mb-3">
                  ...
                  <View className={`px-3 py-1 rounded-lg border ${getStatusColor(request.status)}`}>
                    <Text className="text-xs font-bold uppercase">{request.status}</Text>
                  </View>
                </View>

                {request.reason && (
                  <View className="bg-gray-800 rounded-lg p-3 mb-3">
                    <Text className="text-gray-300 text-xs">{request.reason}</Text>
                  </View>
                )}
                ...
                {request.admin_notes && (
                  <View className="bg-green-900/30 rounded-lg p-3 mb-3 border border-green-800">
                    ...
                    <Text className="text-green-300 text-xs">{request.admin_notes}</Text>
                  </View>
                )}
      ...
      >
        <View className="flex-1 bg-black/70 justify-end">
          ...
          <View className="bg-gray-900 rounded-t-3xl p-6 border-t border-gray-800">
            <View className="flex-row justify-between items-center mb-6">
              <Text className="text-white font-bold text-xl">Request Stock</Text>
            ...
            <View className="mb-4">
              ...
              />
              <Text className="text-gray-500 text-xs mt-1">Min: 1 | Max: 1,000</Text>
            </View>
